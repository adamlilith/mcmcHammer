...
#' @param param Name of the variable(s). The outcome depends on the definitions of \code{i}, \code{j}, \code{k}, and \code{l}. Please see the help for \code{\link{emc_param}} for more explanation on how to specify this argument, plus \code{i}, \code{j}, \code{k}, and \code{l}.
...
emc_trace_dens <- function(
	...
	### compile mcmc
	if (!mcmc_stacked) mcmc <- emc_stack(mcmc)
	...
	for (valid_var in valid_params) {
		...
		if (density) {
			...
			graphs[[count_graph]] <- graphs[[count_graph]] + 
				ggplot2::ggplot(tall[tall$param == valid_var]) +
