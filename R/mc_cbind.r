...
#'
#' @param summarize Logical: If `TRUE` (default), and the input is an `mcmc.list` or list of lists, then calculate summary statistics.
#'
...
mc_cbind <- function(..., summarize = TRUE) {
	...
	if (length(mcmcs) > 1) {
		...
		# for each input
		for (count_mcmc in 2:length(mcmcs)) {
		
			this <- mcmcs[[count_mcmc]]
			if (format == 'mcmc') {

				if (nrow(out) != nrow(this)) stop('Input ', count_mcmc, ' does not have same number of rows as preceding inputs.')

				this_names <- colnames(this)
				out_names <- colnames(out)
				if (any(this_names %in% out_names)) stop('Duplicated names in input ', count_mcmc, '.')

				out <- cbind(out, this)

			} else if (format ==  'mcmc.list') {
				
				if (length(out) != length(this)) stop('Input ', count_mcmc, ' does not have the same number of chains as the preceding inputs.')
				
				for (chain in seq_along(out)) {

					if (nrow(out[[chain]]) != nrow(this[[chain]])) stop('Input ', count_mcmc, ', chain ', chain, ' does not have same number of rows as preceding inputs.')

					this_names <- colnames(this[[chain]])
					out_names <- colnames(out[[chain]])
					if (any(this_names %in% out_names)) stop('Duplicated names in item ', count_mcmc, ', chain ', chain, '.')
				...
				if (length(out[[1]]) != length(this[[1]])) stop('Input ', count_mcmc, ' does not have the same number of chains as the preceding inputs.')
				
				for (chain in seq_along(out[[1]])) {

					if (nrow(out[[1]][[chain]]) != nrow(this[[1]][[chain]])) stop('Input ', count_mcmc, ' does not have same number of rows as preceding inputs.')
